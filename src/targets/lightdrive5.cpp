...
static MidiEvent lastMidiEvent;
static constexpr uint8_t kPPQN = 24;   // MIDI clock pulses per quarter note
static volatile bool gClockRunning = false;
static uint8_t gClockTickInBeat = 0;   // 0..23
...
static inline void handleMidi()
{
    while (usbMIDI.read()) {
        ...
        // --- MIDI realtime transport/clock ---
        if (type == usbMIDI.Start) {
            gClockRunning = true;
            gClockTickInBeat = 0;

            // Optional: treat Start as an immediate downbeat
            animation_tick[SNARE_EVENT_IDX] = 0;
            continue;
        }

        if (type == usbMIDI.Stop) {
            gClockRunning = false;
            continue;
        }

        if (type == usbMIDI.Continue) {
            gClockRunning = true;
            continue;
        }
        ...
        if (type == usbMIDI.Clock) {
            if (!gClockRunning) {
                // Some sources might tick while stopped; ignore unless running.
                gClockRunning = true; // DEBUG;
                continue;
            }

            // Advance within the quarter note (24 ticks)
            gClockTickInBeat++;

            if (gClockTickInBeat >= kPPQN) {
                gClockTickInBeat = 0;

                // Quarter note boundary -> trigger snare animation
                animation_tick[SNARE_EVENT_IDX] = 0;
                // Optional debug:
                Serial.println("SNARE (quarter note)");
            }

            continue;
        }

        // --- Channel messages (notes/CC/etc.) ---
        lastMidiEvent.type         = type;
        lastMidiEvent.note         = usbMIDI.getData1();
        lastMidiEvent.velocity     = usbMIDI.getData2();
        lastMidiEvent.timestamp_ms = millis();

        // Note mapping: NoteOn velocity > 0
        if (lastMidiEvent.type == usbMIDI.NoteOn && lastMidiEvent.velocity > 0) {
